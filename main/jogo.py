...
def esquiva_do_monstro(monstro, personagem):
    ...
    if resultado_esquiva >= personagem["ataque"]+5:
...
def combate(personagem, tipo_monstro):
    ...
    while monstro["vida"] > 0 and personagem["vida"] > 0:
        escrever_devagar("Escolha sua ação: A=Atacar, D=Defender, E=Esquivar, F=Fugir, M=Ver Atributos do Monstro")
        ...
        if acao == "F":
            ...
            escrever_devagar(f"Atributos do {tipo_monstro}:")
